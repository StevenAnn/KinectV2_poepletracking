...
//  depthColorShow.create(480,640,CV_8UC3);;
  depthColorShow.create(640, 480, CV_8UC3);;
  int color_cols;
  int color_rows;
  int counter_out = 0; int counter_in = 0; int counter_no_detection = 0;
  Scalar Colors[]={Scalar(255,0,0),Scalar(0,255,0),Scalar(0,0,255),Scalar(255,255,0),Scalar(0,255,255),Scalar(255,0,255),Scalar(255,127,255),Scalar(127,0,255),Scalar(127,0,127)};
...
// now reading the video stream and then converting it to opencv   
  ...
  {    
	...
	std::string name(b);
	std::ostringstream ss_depth; ss_depth << image_index; name += ss_depth.str();
	name += ext;

	std::string name1(b1);
	std::ostringstream ss_color; ss_color << image_index; name1 += ss_color.str(); image_index++;
	name1 += ext;
...
//复制
	 ...
	 rectangle(color_frame_show, cv::Point(10, 460), cv::Point(100, 480), cv::Scalar(255, 255, 255), -1);
    stringstream ss, f_no;
    ss << (centers.size());
    f_no << frame_num;
    frame_num++;
	...
	putText(color_frame_show, frameNumberString.c_str(), cv::Point(15, 470), FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
	line(color_frame_show, cv::Point(0, 200), cv::Point(639, 200), Scalar(0, 0, 255), .2, CV_AA);
	line(color_frame_show, cv::Point(0, 230), cv::Point(639, 230), Scalar(255, 0, 0), .2, CV_AA);
...
//			line(color_frame, tracker.tracks[i]->trace[j], tracker.tracks[i]->trace[j + 1], Colors[tracker.tracks[i]->track_id % 9], 2, CV_AA);
    ...
    string countInString = countIn.str();
    string countOutString = countOut.str();
	putText(color_frame_show, countInString.c_str(), cv::Point(550, 15), FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 255));
	putText(color_frame_show, countOutString.c_str(), cv::Point(550, 460), FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 0, 0));
